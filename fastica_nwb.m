function [Out1, Out2, Out3, NonConvergence] = fastica_nwb(mixedsig, varargin) % NWB adjusted to output "NonConvergence=1" if fastica_symm does not converge (which causes issues with ICA cleaning)
...
if only > 2
  ...
  [A, W, NonConvergence] = fpica_nwb (whitesig,  whiteningMatrix, dewhiteningMatrix, approach, ...
		  numOfIC, g, finetune, a1, a2, myy, stabilization, epsilon, ...
		  maxNumIterations, maxFinetune, initState, guess, sampleSize, ...
		  displayMode, displayInterval, verbose); % NWB adjusted to output "NonConvergence=1" if fastica_symm does not converge (which causes issues with ICA cleaning)
