...
function [A, W, NonConvergence] = fpica_nwb(X, whiteningMatrix, dewhiteningMatrix, approach, ...
			...
			s_verbose); % NWB adjusted to output "NonConvergence=1" if fastica_symm does not converge (which causes issues with ICA cleaning)
...
NonConvergence=0; % NWB adjusted to output "NonConvergence=1" if fastica_symm does not converge (which causes issues with ICA cleaning)
  ...
  while round <= numOfIC,
	...
	if i == maxNumIterations + 1
	  ...
	  if numFailures > failureLimit
          NonConvergence=1; % NWB adjusted to output "NonConvergence=1" if fastica_symm does not converge (which causes issues with ICA cleaning)
	    if b_verbose
